...
import { type HandbookFile } from "@/lib/handbook";
import {
  handbookStorage,
...
const gateway = "shared_improvements";
...
export async function GET(request: Request) {
  ...
  if (!handbookStorageConfigured())
    return json({ error: "Receipt file storage is not configured." }, 503);
  try {
    ...
    logApiFailure("/api/receipt/files", "get", err);
    return json({ error: "Could not open that receipt file." }, 404);
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const entryId = String(form.get("expense_id") || "");
    ...
    const { error: uploadError } = await handbookStorage(
      "expense-receipts",
    ).upload(path, Buffer.from(await uploaded.arrayBuffer()), {
    ...
    try {
      ...
      await handbookStorage("expense-receipts").remove([path]);
      throw err;
    }
  } catch (err) {
    console.error("POST /api/receipt/files", {
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    if (typeof id !== "string")
      return json({ error: "Choose a receipt file." }, 400);
    const result = await sharedDatabase(
      "remove_receipt",
      { actor: member, id },
      gateway,
    ...
    const file = result.file as HandbookFile;
    const { error } = await handbookStorage("expense-receipts").remove([
      file.storage_path,
    ]);
    if (error)
      console.error("Receipt object removal failed", {
        code: error.statusCode,
      });
    broadcastChange("home", null);
    return json({ ok: true });
  } catch (err) {
    logApiFailure("/api/receipt/files", "delete", err);
