-- Recurring entries: repeating plans are created as real rows up front,
-- linked by series_id so update/delete can target every occurrence at once.
begin;
alter table public.entries add column series_id uuid;
...
declare hid uuid; owner_uid uuid; row_data public.entries; affected integer; n integer; person_id uuid;
  step interval; until_date date; occurrence date; sid uuid;
begin
  ...
  if operation = 'attempt' then
    ...
    if step is null or occurrence is null then
      ...
      values(hid,owner_uid,payload->>'kind',trim(payload->>'title'),coalesce(payload->>'description',''),payload->>'category',
        occurrence,nullif(payload->>'assignee','')::uuid,nullif(payload->>'amount','')::numeric,coalesce(payload->>'url',''));
    else
      ...
      while occurrence <= until_date loop
        ...
        values(hid,owner_uid,payload->>'kind',trim(payload->>'title'),coalesce(payload->>'description',''),payload->>'category',
          occurrence,nullif(payload->>'assignee','')::uuid,nullif(payload->>'amount','')::numeric,coalesce(payload->>'url',''),sid);
        n := n + 1;
