-- Alternating chores, per-occurrence payments, and short-lived delete undo.
begin;
alter table public.entries
  add column rotation_members uuid[] not null default '{}',
  add column payment_members uuid[] not null default '{}',
  add column paid_by uuid[] not null default '{}';
...
alter table public.entries add constraint entries_category_check
  check (category in ('Chore','To-do','Together','Rent','Bill','Other','Need','Want','Note'));
alter table public.entries add constraint entries_paid_members_check check (paid_by <@ payment_members);
-- Existing rent reminders get checks for the current household members.
update public.entries e set payment_members = array(
  select m.user_id from public.members m where m.household_id=e.household_id and m.name <> 'Housemates' order by m.name, m.user_id
) where e.kind='event' and e.category='Rent';
create table public.deleted_entry_batches (
  id uuid primary key,
  household_id uuid not null references public.households(id),
  deleted_by uuid not null references public.members(user_id),
  entries jsonb not null,
  expires_at timestamptz not null default now() + interval '30 seconds'
);
alter table public.deleted_entry_batches enable row level security;
revoke all on public.deleted_entry_batches from public, anon, authenticated;
...
declare hid uuid; owner_uid uuid; row_data public.entries; affected integer; n integer; person_id uuid;
  ...
  rotation uuid[] := '{}'; payers uuid[] := '{}'; partner uuid; token uuid; deleted_rows jsonb;
begin
  ...
  if operation = 'attempt' then
    ...
    if row_data.kind='event' and (payload ? 'category') and payload->>'category' <> row_data.category
      and (row_data.category in ('Rent','Bill') or payload->>'category' in ('Rent','Bill')) then
      -- Switching Rent <-> Bill keeps payments; leaving bills must not discard paid records.
      if row_data.category in ('Rent','Bill') and payload->>'category' not in ('Rent','Bill') and exists(
      ...
      payers := array(select user_id from public.members where household_id=hid and name <> 'Housemates' order by name,user_id);
      update public.entries e set payment_members=case when payload->>'category' in ('Rent','Bill') then
        case when cardinality(e.payment_members)>0 then e.payment_members else payers end else '{}'::uuid[] end
